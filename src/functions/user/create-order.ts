...
export async function CODorder(
  products: products,
  addressId: string,
  useStoreCredit: boolean,
) {
  ...
  const findAddress = await db.query.address.findFirst({
    where: (address, o) =>
      o.and(o.eq(address.userId, session.user.id), o.eq(address.id, addressId)),
  });
  ...
  const ids = products.map((product) => product.productWithQuantity.id);

  const productList = await db.select().from(product).where(inArray(product.id, ids));
  ...
  if (amountToPay <= 0) {
    // invalid cod order
    return ErrorResponse("Invalid COD Order");
  }

  const id = uuid();
  const rzpOrderId = uuid();

  await db.transaction(async (tx) => {
    // Check stock for all items atomically
    for (const product of products) {
      const quantityRecord = await tx.query.quantity.findFirst({
        where: (q, o) => o.eq(q.productId, product.productWithQuantity.id),
      });
    ...
    const { id, userId, createdAt, updatedAt, ...address } = findAddress;
  ...
  const allOrders = await db.query.order.findMany({
    where: (o) => eq(o.rzpOrderId, rzpOrderId),
    with: { user: true, product: true },
  });
  ...
  //       pin: zipcode,
  //       payment_mode: "COD",
  //       cod_amount: totalAmount.toFixed(2),
  //       weight: totalWeight,
  //       shipment_height: totalHeight,
  //       shipment_length: totalLength,
  //       shipment_width: totalWidth,
  ...
  // console.log("CREATE SHIPMENT: ", createShipment);
  ...
  //     waybill,
  //   })
  //   .where(eq(order.rzpOrderId, rzpOrderId));
  // const paymentId = uuid();

...
export const createCouponCodeSchema = z.object({
  ...
  firstOrder: z.boolean("firstOrder is required"),
...
export const CreateCheckoutUser = z.object({
  ...
  phone: z
    ...
    .max(10, "Phone Number must be 10 digits")
    .regex(/^[6-9]\d{9}$/, "Invalid Phone Number"),
  address1: z.string().min(30, "Address too short").max(150, "Address too long").trim(),
  address2: z.string().max(60, "Address too long").trim().optional(),
